...
<body class="h-screen bg-white flex flex-col items-center">
    ...
    <main class="w-full max-w-screen-lg mt-8 px-4">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <h1 class="text-3xl font-bold text-center flex-grow">Editar veterinario</h1>
        </div>

        <!-- Formulario de registro de mascota -->
        <form class="grid grid-cols-2 gap-6" method="POST" enctype="multipart/form-data" action="{{ route('veterinario.update', $veterinario->id) }}"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label for="nombre" class="block text-sm font-medium text-gray-700">Nombre del veterinario:</label>
                <input type="text" id="nombre" name="nombre" value="{{ $veterinario->nombre }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    placeholder="Nombre del veterinario" required>
            </div>
            <div>
                <label for="descripcion" class="block text-sm font-medium text-gray-700">Descripción:</label>
                <input type="text" id="descripcion" name="descripcion" value="{{ $veterinario->descripcion }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    placeholder="Descripción" min="0" required>
            </div>
            <div>
                <label for="ubicacion" class="block text-sm font-medium text-gray-700">Ubicación:</label>
                <input type="text" id="ubicacion" name="ubicacion" value="{{ $veterinario->ubicacion }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    placeholder="Ubicacion" required>
            </div>
            <div>
                <label for="horario" class="block text-sm font-medium text-gray-700">Horario:</label>
                <input type="text" id="horario" name="horario" value="{{ $veterinario->horario }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    placeholder="Horario" required>
            </div>
            <div>
                <label for="telefono" class="block text-sm font-medium text-gray-700">Telefono:</label>
                <input type="text" id="telefono" name="telefono" value="{{ $veterinario->telefono }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
